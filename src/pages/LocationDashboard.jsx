...
function GlowOrb({ x, y, color, size = 400, opacity = 0.08 }) {
  return (
    <div style={{
      ...
      borderRadius: "50%", background: color, opacity, filter: "blur(110px)",
...
function Sidebar({ navigate }) {
  return (
    <div style={{
      ...
      background: COLORS.bg.card, borderRight: "1px solid rgba(127, 119, 221, 0.1)",
      display: "flex", flexDirection: "column", zIndex: 50, padding: "1.5rem 1rem",
    }}>
      ...
      {[
        ...
        return (
          <div key={item.label} onClick={() => navigate(item.p)} style={{
            display: "flex", alignItems: "center", gap: 10, padding: "0.6rem 0.8rem",
            ...
            {item.label}
            {active && <div style={{ marginLeft: "auto", width: 5, height: 5, borderRadius: "50%", background: COLORS.primary.light }} />}
          </div>
        );
      })}
...
function StatCard({ label, value, color, icon }) {
  return (
    <div style={{
      background: COLORS.bg.card, border: "1px solid rgba(127, 119, 221, 0.1)",
      borderTop: `2px solid ${color}`, borderRadius: 12, padding: "1.1rem 1.3rem", textAlign: "center",
    }}>
...
function Badge({ etat }) {
  ...
  return (
    <span style={{
      ...
      fontSize: "0.72rem", fontWeight: 600, padding: "0.22rem 0.7rem", borderRadius: 100,
    }}>
...
function FormField({ label, value, onChange, type = "text", options }) {
  ...
  const base = {
    ...
    borderRadius: 8, padding: "0.72rem 1rem", fontSize: "0.88rem",
    color: COLORS.text.primary, outline: "none", fontFamily: "inherit",
    transition: "all 0.2s", boxShadow: focused ? `0 0 0 3px ${COLORS.primary.light}18` : "none",
  };
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: "block", fontSize: "0.75rem", color: "#7C7A99", marginBottom: 6, fontWeight: 500, letterSpacing: "0.05em", textTransform: "uppercase" }}>
        {label}
      </label>
      {options ? (
...
function PrimaryBtn({ children, onClick, disabled, danger }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      border: "none", borderRadius: 8, padding: "0.72rem 1.4rem", fontSize: "0.88rem",
      fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", fontFamily: "inherit",
      transition: "all 0.18s", boxShadow: hov && !disabled ? `0 0 20px ${danger ? COLORS.accent.coral : COLORS.primary.light}44` : "none",
    }}>
      {children}
    </button>
  );
...
function SensorRow({ sensor, last, onEdit, onDelete }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      <span style={{ fontSize: "0.85rem", color: COLORS.text.muted }}>{sensor.zone}</span>
      <Badge etat={sensor.etat} />
      <span style={{
        display: "inline-flex", alignItems: "center", gap: 5,
        background: sensor.accidentSignale ? `${COLORS.accent.coral}18` : "rgba(255,255,255,0.04)",
        border: `1px solid ${sensor.accidentSignale ? COLORS.accent.coral + "44" : "rgba(255,255,255,0.08)"}`,
        color: sensor.accidentSignale ? "#F0997B" : COLORS.text.muted,
        fontSize: "0.7rem", fontWeight: 600, padding: "0.22rem 0.65rem", borderRadius: 100,
      }}>
...
export default function LocationDashboard() {
  ...
  const emptyForm = { name: "", zone: ZONES[0], etat: "ACTIF", accidentSignale: false };
  ...
  // ── Search: /api/users/sensorId/{id} | /api/users/sensorName/{name} | /api/users/sensorZone/{zone} ──
  const handleSearch = async () => {
    ...
    } catch (e) { console.error(e); showToast("Erreur de recherche", "error"); }
    finally { setSearching(false); }
  };

  const clearSearch = () => { setSearchVal(""); setSearchResult(null); };

  // ── POST /api/admin/newSensor ──
  ...
  const openEdit = (sensor) => {
    setForm({ name: sensor.name, zone: sensor.zone, etat: sensor.etat || "ACTIF", accidentSignale: sensor.accidentSignale || false });
    setShowEdit(sensor);
  };
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: COLORS.bg.main, color: COLORS.text.primary, minHeight: "100vh" }}>
      ...
      {toast && (
        <div style={{
          ...
          borderRadius: 10, padding: "0.75rem 1.2rem", fontSize: "0.85rem", fontWeight: 500,
      ...
      <div style={{ marginLeft: 220, padding: "2.5rem", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14, marginBottom: "2rem" }}>
          ...
          ].map(stat => <StatCard key={stat.label} label={stat.label} value={stat.value} color={stat.color} icon={stat.icon} />)}
        </div>
        ...
        <div style={{ background: COLORS.bg.card, border: "1px solid rgba(127, 119, 221, 0.1)", borderRadius: 14, padding: "1.2rem 1.4rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex", background: COLORS.bg.hover, borderRadius: 8, padding: 3, gap: 2 }}>
            {["name", "id", "zone"].map((type) => (
              <button key={type} onClick={() => { setSearchType(type); clearSearch(); }} style={{
                ...
                border: "none", borderRadius: 6, padding: "0.38rem 0.9rem", fontSize: "0.82rem",
          ...
          <input value={searchVal} onChange={e => setSearchVal(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSearch()}
            placeholder={searchPlaceholders[searchType]} style={{
              flex: 1, minWidth: 180, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)",
              borderRadius: 8, padding: "0.65rem 1rem", fontSize: "0.88rem", color: COLORS.text.primary, outline: "none", fontFamily: "inherit",
            }} />
          ...
          {searchResult && (
            <button onClick={clearSearch} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#7C7A99", borderRadius: 8, padding: "0.65rem 1rem", fontSize: "0.82rem", cursor: "pointer", fontFamily: "inherit" }}>
              Clear ✕
            </button>
          )}
        </div>
        ...
        <div style={{ background: COLORS.bg.card, border: "1px solid rgba(127, 119, 221, 0.1)", borderRadius: 14, overflow: "hidden" }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: "3rem", textAlign: "center", color: COLORS.text.muted, fontSize: "0.88rem" }}>No sensors found</div>
          ) : (
            displayed.map((s, i) => (
              <SensorRow key={s.id ?? i} sensor={s} last={i === displayed.length - 1} onEdit={openEdit} onDelete={setShowDelete} />
            ))
          )}
        </div>
      </div>
      ...
      {showCreate && (
        <Modal title="New Sensor" onClose={() => setShowCreate(false)}>
          ...
          <FormField label="Zone" value={form.zone} onChange={e => setForm({ ...form, zone: e.target.value })} options={ZONES} />
      ...
      {showDelete && (
        <Modal title="Delete Sensor" onClose={() => setShowDelete(null)}>
          <p style={{ color: "#9CA3AF", fontSize: "0.9rem", lineHeight: 1.7, marginBottom: "1.5rem" }}>
            Are you sure you want to delete sensor <span style={{ color: COLORS.text.primary, fontWeight: 600 }}>{showDelete.name}</span>? This action cannot be undone.
          </p>
