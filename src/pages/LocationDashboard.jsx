    ...
    <div style={{ marginBottom: 14 }}>
      ...
      {options ? (
        <select value={value} onChange={onChange} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={{ ...base, cursor: "pointer" }}>
          {options.map(o => <option key={o} value={o} style={{ background: C.bg2 }}>{o}</option>)}
        </select>
      ) : (
...
export default function LocationDashboard() {
  ...
  // forms
  const emptyForm = { name: "", zone: "", etat: "ACTIF" };
  ...
  const handleSearch = async () => {
    if (!searchVal.trim()) { setSearchResult(null); return; }
    setSearching(true);
    try {
      const res = await fetch(
        searchType === "id"
          ? `${API}/api/admin/sensor/${searchVal.trim()}`
          : `${API}/api/admin/sensors`,
        { headers: authHeaders() }
      );
      if (!res.ok) { showToast("Capteur introuvable", "error"); setSearchResult(null); return; }
      const data = await res.json();
      if (searchType === "id") {
        setSearchResult([data]);
      } else {
        const filtered = data.filter(s =>
          s.name?.toLowerCase().includes(searchVal.trim().toLowerCase())
        );
        setSearchResult(filtered.length ? filtered : null);
        if (!filtered.length) showToast("Aucun résultat", "error");
      }
  ...
  const handleCreate = async () => {
    try {
      ...
      showToast("Capteur créé avec succès");
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/api/admin/sensor/${showDelete.id}`, {
        method: "DELETE", headers: authHeaders(),
      });
      showToast("Capteur supprimé");
      setShowDelete(null); fetchAll();
    } catch { showToast("Erreur lors de la suppression", "error"); }
  };

  const openEdit = (s) => { setForm({ name: s.name, zone: s.zone, etat: s.etat }); setShowEdit(s); };
    ...
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: C.bg, color: "#f0f0f0", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        ::-webkit-scrollbar{width:5px;} ::-webkit-scrollbar-track{background:transparent;} ::-webkit-scrollbar-thumb{background:#534AB755;border-radius:4px;}
      `}</style>
      ...
      {toast && (
        <div style={{
          ...
          border: `1px solid ${toast.type === "error" ? C.coral : C.t400}44`,
      ...
      <div style={{
        ...
        {[
          { label: "Locations", icon: "📡", active: true },
          { label: "Traffic Data", icon: "📊", active: false },
          { label: "Congestion", icon: "🧠", active: false },
          { label: "Statistics", icon: "📈", active: false },
        ].map(item => (
      ...
      {/* ── MAIN ── */}
      <div style={{ marginLeft: 220, padding: "2.5rem 2.5rem", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            ...
            <p style={{ color: "#4A4868", fontSize: "0.88rem", marginTop: 4, fontWeight: 300 }}>
              Manage your traffic sensor locations
            </p>
          </div>
          <PrimaryBtn onClick={openCreate}>+ New Location</PrimaryBtn>
        </div>

        {/* STATS ROW */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 14, marginBottom: "2rem" }}>
          {[
            { label: "Total Locations", value: sensors.length, color: C.p400 },
        ...
        <div style={{
          ...
          <input
            ...
            placeholder={searchType === "id" ? "Enter location ID..." : "Search by name..."}
            style={{
              flex: 1, minWidth: 180,
        ...
        <div style={{
          ...
          {/* TABLE HEADER */}
          <div style={{
            display: "grid", gridTemplateColumns: "60px 1fr 1fr 120px 160px",
      ...
      {showCreate && (
        <Modal title="New Location" onClose={() => setShowCreate(false)}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <PrimaryBtn onClick={handleCreate} disabled={!form.name || !form.zone}>Create</PrimaryBtn>
          </div>
        </Modal>
      )}
      ...
      {showEdit && (
        <Modal title={`Edit — ${showEdit.name}`} onClose={() => setShowEdit(null)}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <PrimaryBtn onClick={handleUpdate} disabled={!form.name || !form.zone}>Save Changes</PrimaryBtn>
          </div>
        </Modal>
      )}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <ActionBtn onClick={onEdit} color={C.p400} label="Edit" />
